    ...
    try:
        ...
        with engine.connect():
            ...
            if table_name not in inspector.get_table_names():
                raise SchemaIntrospectionError(f"Table '{table_name}' not found in database")
...
def get_database_schema(connection_string: str, tables: Optional[List[str]] = None) -> Dict[str, Any]:
...
def _convert_table_to_json_schema(
    ...
    for column in columns:
        ...
        if col_name in primary_keys:
            json_property["description"] = f"Primary key - {json_property.get('description', '')}"
            json_property["primary_key"] = True
            
        if col_name in foreign_keys:
            fk_info = foreign_keys[col_name]
            json_property["description"] = f"Foreign key to {fk_info['referenced_table']}.{fk_info['referenced_column']} - {json_property.get('description', '')}"
            json_property["foreign_key"] = fk_info
            json_property["type"] = "ref"
            json_property["ref"] = f"{fk_info['referenced_table']}.{fk_info['referenced_column']}"
...
def _sql_type_to_json_schema(sql_type: str, column_name: str) -> Dict[str, Any]:
    ...
    if any(t in sql_type for t in ['varchar', 'char', 'text', 'string']):
        ...
        if 'email' in col_lower:
            ...
            if length_match:
                max_length = min(int(length_match.group(1)), 100)  # Cap at 100 for generation
